    ...
    (set, get) => ({
      ...
      linkSeriesFromDate: (taskId, fromDate, linked) =>
        set((s) => {
          ...
          if (!linked) {
            // Collect the dates that must never be regenerated by the parent's
            // recurrence pattern (the source occurrence + any future instances
            // we're about to drop). Prefer originalDate (the slot the pattern
            // scheduled) over task.date (which may have been user-moved).
            const exceptionDates = new Set<string>();
            for (const task of s.tasks) {
              if (!isTaskInSameSeries(task, seriesId)) continue;
              if (task.id === taskId) {
                exceptionDates.add(task.originalDate || task.date);
              } else if (task.date >= fromDate) {
                exceptionDates.add(task.originalDate || task.date);
              }
            }
            ...
            for (const task of s.tasks) {
              ...
              if (task.id === taskId) {
                ...
                // Parent (or past instance) — record exceptions on the parent
                // so future regeneration doesn't recreate the dropped slots.
                const isParent = !task.isRecurrenceInstance && !!task.recurrence;
                if (isParent && exceptionDates.size > 0) {
                  const prev = new Set(task.recurrenceExceptions || []);
                  for (const d of exceptionDates) prev.add(d);
                  remaining.push({ ...task, recurrenceExceptions: Array.from(prev).sort() });
                } else {
                  remaining.push(task);
                }
              }
