...
function FocusDayListPanel({ allTodayTasks, completedCount, nowMinutes, activeTaskId, onTaskTap, onBack }: FocusDayListPanelProps) {
  ...
  return (
    <div className="absolute inset-0">
      ...
      >
        {/* Header */}
        <div className="max-w-sm sm:max-w-2xl mx-auto w-full mb-2">
        ...
        {/* Task list */}
        <div className="max-w-sm sm:max-w-2xl mx-auto w-full flex flex-col">
          {allTodayTasks.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-muted-foreground/30 font-mono text-sm tracking-wider">NO TASKS</p>
            </div>
          ) : (
            allTodayTasks.filter((t) => !t.groupId).map((task) => {
              if (task.type === 'group') {
                return (
                  <GroupListRow
                    key={task.id}
                    group={task}
                    onGroupTap={onTaskTap}
                    renderChild={(child) => renderFocusRow(child, false, true)}
                  />
                );
              }
              ...
              return (
                <div
                  ...
                  className={isCurrent ? 'border border-foreground/15 rounded-md my-1' : ''}
                >
                  {renderFocusRow(task, isCurrent, false)}
                </div>
              );
            })
          )}
...
function TaskDetailPanel({ task, onUpdateTask, onCompleteTask }: TaskDetailPanelProps) {
  ...
  return (
    <div className="w-full max-w-sm sm:max-w-2xl mx-auto flex flex-col gap-5">
