...
export function LibraryDetailPane({ item, onClose }: LibraryDetailPaneProps) {
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header */}
      <div className="px-5 pt-4 pb-1 flex items-center justify-between border-b border-border/30">
        <span className="text-[10px] font-mono tracking-[0.14em] text-muted-foreground/50">DETAIL</span>
        <button
          onClick={onClose}
            ...
            {attachments.filter(a => !a.type.startsWith('image/')).map((att, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2 py-1.5 group">
                  ...
                  <button onClick={() => setLightboxIndex(realIndex)} className="flex-1 text-left text-[10px] font-mono text-foreground/60 hover:text-foreground truncate">
                    {att.name}
                  </button>
