...
export function BillableTimeBreakdown() {
  ...
  const renderRow = (r: TagRow, depth: number) => {
    ...
    const displayLabel = depth > 0
      ...
      : r.label;
    // Each bar is normalized to this tag's own total minutes — 100% width = this tag's tracked time.
    // That way "blue = billed share, dark = unbilled share" reads at a glance.
    const billedClamped = Math.min(r.billedMinutes, r.minutes);
    const unbilledMinutes = Math.max(0, r.minutes - billedClamped);
    const billedPct = r.minutes > 0 ? (billedClamped / r.minutes) * 100 : 0;
    const unbilledPct = r.minutes > 0 ? (unbilledMinutes / r.minutes) * 100 : 0;
